...
var MaterialUiIconPicker = (function (_React$Component) {
	...
	function MaterialUiIconPicker(props) {
		...
		var iconObj = this.iconsStorage.getIcons();
	...
	_createClass(MaterialUiIconPicker, [{
		...
		value: function filterList(event) {
			...
			if (event.target.value.toLowerCase().length === 0) {
				...
				updatedList = updatedList.filter(function (item) {
					var searchString = item.code + ' ' + item.name;
					var searches = searchString.split('-').map(function (namePiece) {
		...
		value: function render() {
			...
			var actions = [_react2['default'].createElement(_materialUi.FlatButton, {
				...
				icon: this.state.selected ? _react2['default'].createElement('i', { className: 'fa ' + this.state.selected.code }) : null
			})];
			var icons = Object.keys(this.state.icons).map(function (index, icon, lame) {
				icon = _this.state.icons[index];
				return _react2['default'].createElement(
					'div',
					{ key: index, style: styles.iconsItem, onClick: function () {
							return _this.select(icon);
						} },
					_react2['default'].createElement('div', {
						style: _this.state.selected && _this.state.selected.code === icon.code ? styles.selectedBackgroundBox : styles.backgroundBox }),
					_react2['default'].createElement('i', { className: 'fa ' + icon.code, style: { fontSize: 32, zIndex: 1 } }),
					_react2['default'].createElement(
						'div',
						{ style: styles.iconsItemCaption },
						icon.code.split('-').join(' ').substr(3)
					)
				);
			});
			...
			return _react2['default'].createElement(
				...
				_react2['default'].createElement(
					...
					this.state.pickerDialogOpen ? _react2['default'].createElement(
						_materialUi.Dialog,
						{
